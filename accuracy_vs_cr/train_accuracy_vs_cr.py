...
def train_acc_vs_cr(NN,epochs,criterion,optimizer,scheduler,cr,cv,train_loader,validation_loader,path,device = 'cpu',net_name = 'Lenet5'):

    top1_metric = Accuracy().to(device)
    top5_metric = Accuracy(top_k = 5).to(device)

    running_data = pd.DataFrame(data=None, columns=['epoch', 'cr layerwise','cross val' ,'learning_rate', 'train_loss', 'train_accuracy(%)',
                                                    'top_5_accuracy(%)', \
    ...
    for epoch in tqdm(range(epochs)):
        ...
        cr_print = cr if isinstance(cr,str) else round(1-cr,4)
        epoch_data = [epoch, cr_print ,cv, round(lr, 5), round(loss_hist, 3), round(acc_hist * 100, 4),
        ...
        if file_name is not None:
            running_data.to_csv(file_name +'_running_data_'+net_name + f'_{str(cv)}_{str(round(cr,2))}_'+ '.csv')



def train_baseline(NN,epochs,criterion,optimizer,scheduler,cr,cv,train_loader,validation_loader,path,device = 'cpu',net_name = 'Lenet5'):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if file_name is not None:
            running_data.to_csv(file_name +'_running_data_'+net_name + f'_{str(cv)}_{str(cr)}_'+ '.csv')