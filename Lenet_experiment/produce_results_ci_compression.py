...
cross_val_tab = pd.DataFrame(data = None,columns = ['theta','test accuracy mean','test accuracy std','ranks',\
                                                    'params test','cr test mean ','cr test std',\
                                                    'params train','cr train mean','cr train std',
                                                    'cr train no grads mean','cr train no grads std'
                                                    ])
...
for i,theta in enumerate([0.07,0.09,0.11,0.13,0.15,0.2,0.3,0.4,0.45]):
  ...
  cross_val_tab.loc[i] = [theta,
                          ...
                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_test (%)'].mean(),3),
                          
                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_test (%)'].std(),3),
                          
                          int(cv_all_results.loc[(cv_all_results['theta']==theta) & (cv_all_results['test_accuracy(%)']==\
                                              max(cv_all_results[cv_all_results['theta']==theta]['test_accuracy(%)']))]['# effective parameters train with grads']),
                          
                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_train_grads (%)'].mean(),3),
                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_train_grads (%)'].std(),3),

                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_train (%)'].mean(),3),
                          np.round(cv_all_results.loc[cv_all_results['theta']==theta]['cr_train (%)'].std(),3),
                          ]
...
baseline_row = ['Baseline',\
                          ...
                          np.round(cv_baseline['cr_test (%)'].mean(),3),
                          np.round(cv_baseline['cr_test (%)'].std(),3),
                          
                          
                          int(cv_baseline.loc[(cv_baseline['test_accuracy(%)']==\
                                              max(cv_baseline['test_accuracy(%)']))]['# effective parameters train with grads']),
                          
                          np.round(cv_baseline['cr_train_grads (%)'].mean(),3),
                          np.round(cv_baseline['cr_train_grads (%)'].std(),3),

                          np.round(cv_baseline['cr_train (%)'].mean(),3),
                          np.round(cv_baseline['cr_train (%)'].std(),3),
                          ]

cross_val_tab.loc[0] = baseline_row

# save table
pd.DataFrame.to_csv(cross_val_tab,'cross_val_tab_paper_ci_compression.csv')
