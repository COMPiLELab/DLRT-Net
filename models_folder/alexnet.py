...
class Flatten(torch.nn.Module):
    def forward(self, input):
        ...
        out = input.reshape(batch_size,-1)
...
class AlexNet(torch.nn.Module):
    def __init__(self, output_dim,device = 'cpu'):
        ...
        self.layer = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            Conv2d_lr(192, 384, 3, padding=1,device = self.device,rank = None),
            ...
            Linear(256 * 2 * 2, 4096,rank = 256*4,device = self.device),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.2),
            Linear(4096, 4096,rank = 4096,device = self.device),
            torch.nn.ReLU(),
...
class AlexNet_all_layers_compress(torch.nn.Module):
    def __init__(self, output_dim,device = 'cpu'):
        super().__init__()
        self.device = device
        self.layer = torch.nn.Sequential(
            Conv2d_lr(in_channels = 3,out_channels = 64,kernel_size= 3,stride =  2, padding = 1,device = self.device,rank = 64),  # in_channels, out_channels, kernel_size, stride, padding
            torch.nn.BatchNorm2d(64),
            torch.nn.MaxPool2d(2),  # kernel_size
            torch.nn.ReLU(),
            Conv2d_lr(64, 192, 3, padding=1,device = self.device,rank = 192),
            torch.nn.BatchNorm2d(192),
            torch.nn.MaxPool2d(2),
            torch.nn.ReLU(),
            Conv2d_lr(192, 384, 3, padding=1,device = self.device,rank = 384),
            torch.nn.BatchNorm2d(384),
            torch.nn.ReLU(),
            Conv2d_lr(384, 256, 3, padding=1,device = self.device,rank = 256),
            torch.nn.BatchNorm2d(256),
            torch.nn.ReLU(),
            Conv2d_lr(256, 256, 3, padding=1,device = self.device,rank = 256),
            torch.nn.BatchNorm2d(256),
            torch.nn.MaxPool2d(2),
            torch.nn.ReLU(),
            Flatten(),
