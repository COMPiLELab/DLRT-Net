...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A',device = 'cpu'):
        ...
        self.device = device
        self.conv1 = Conv2d_lr(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False,rank = min([9*in_planes,planes]),\
                              device = self.device,full_rank_construct=full_rank_reconstruct)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = Conv2d_lr(planes, planes, kernel_size=3, stride=1, padding=1, bias=False,rank = min([9*in_planes,planes]),\
                                device  = self.device,full_rank_construct=full_rank_reconstruct)
            ...
            if option == 'A':
                ...
                self.shortcut = nn.Sequential(
                     Conv2d_lr(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False,\
                               rank = min([in_planes,self.expansion*planes]),
                               device = self.device,full_rank_construct=full_rank_reconstruct),
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10,device = 'cpu'):
        ...
        self.linear0 = Linear(64,64,rank = 64,
                              device = self.device,full_rank_construct=full_rank_reconstruct)
        self.linear1 = Linear(64,64,rank = 64,
                              device = self.device,full_rank_construct=full_rank_reconstruct)
