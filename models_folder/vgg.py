        ...
        for x in architecture:
            if type(x) == int:
                ...
                self.layer.add_module('conv_'+str(j),Conv2d_lr(
                            ...
                            stride=(1,1),
                            padding=(1, 1),
                            rank = None,#min([in_channels*9,out_channels]),  # decomment for low rank also on convolution
...
class VGG_all_layers_compress(nn.Module):
    def __init__(
        self,
        architecture,
        in_channels=3, 
        ...
        super(VGG_all_layers_compress, self).__init__()
        self.device = device
        self.in_channels = in_channels
        self.in_width = in_width
        self.in_height = in_height
        ...
        for x in architecture:
            if type(x) == int:
                ...
                self.layer.add_module('conv_'+str(j),Conv2d_lr(
                            ...
                            stride=(1,1),
                            padding=(1, 1),
                            rank = min([in_channels*9,out_channels]),  # decomment for low rank also on convolution
                            device = self.device,full_rank_construct=full_rank_reconstruct
                        ))
                self.layer.add_module('bn_'+str(j),nn.BatchNorm2d(out_channels))
                self.layer.add_module('relu_'+str(j),nn.ReLU())  
