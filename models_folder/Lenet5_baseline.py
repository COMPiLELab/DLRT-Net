...
class Lenet5(torch.nn.Module):
    def __init__(self,device = 'cpu'):
        ...
        self.layer = torch.nn.Sequential(
            Conv2d_lr(in_channels = 1, out_channels = 20, kernel_size = 5, stride=1,rank = None,device = self.device),  
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size = 2, stride=2),
            Conv2d_lr(in_channels = 20, out_channels = 50, kernel_size = 5, stride=1,rank = None,device = self.device),  
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size = 2, stride=2),
            torch.nn.Flatten(),
            Linear(800,out_features = 500,rank = None,device = self.device),  
            torch.nn.ReLU(),
