...
from models_folder.resnet20 import ResNet20_cifar100
import torch
...
NN = ResNet20_cifar100(device=device).to(device)
# #################### load low rank
# NN.load_state_dict(torch.load('./results_cifar10/_running_data_0.15_best_weights.pt'))
# #################### + load full rank
# NN.to_low_rank()
optimizer = dlr_opt(NN, tau=0.05, theta=0.15,KLS_optim = torch.optim.SGD,momentum = 0.1)
