...
from Lenet5_dlrt import Lenet5
...
############################################## parser creation
parser = argparse.ArgumentParser(description='Pytorch dlrt training for Lenet5 on mnist')
...
parser.add_argument('--decay', type=float, default=1, metavar='DECAY',
                    help='decay of starting singular values (default: 1)')   
...
for cv_run in range(args.cv_runs):
    ...
    f = f.to(device)
    optimizer = dlr_opt(f,tau = args.lr,theta = args.theta,KLS_optim=torch.optim.SGD)
    if d == 10:
        path = './results_vanilla/cv_vanilla/_running_data_dlrt'+str(cv_run)
    elif d == 1:
        path = './results_vanilla/cv_vanilla_baseline/_running_data_dlrt'+str(cv_run)
    elif d == 2:
        path = './results_vanilla/cv_vanilla_exp2/_running_data_dlrt'+str(cv_run)
    ...
    for epoch in tqdm(range(MAX_EPOCHS)):
        ...
        for i, data in enumerate(train_loader):  # train
            ...
            def closure():
                loss = f.populate_gradients(inputs, labels, criterion, step='S')
                return loss


            optimizer.preprocess_step()
            loss,outputs = f.populate_gradients(inputs, labels, criterion)
            loss_hist += float(loss.item()) / k
            outputs = outputs.to(device)#f(inputs).detach().to(device)
        ...
        if file_name is not None:
            if d == 10:
                running_data.to_csv(file_name +'dlrt_lenet_exp10' + '.csv')
            elif d == 1:
                running_data.to_csv(file_name +'dlrt_lenet_baseline' + '.csv')
            elif d == 2:
                running_data.to_csv(file_name +'dlrt_lenet_exp2' + '.csv')

        if epoch == 0:
            best_val_loss = loss_hist_test

        if loss_hist_test < best_val_loss:
            pass#torch.save(f.state_dict(), path +'dlrt_lenet' + '_best_weights.pt')
