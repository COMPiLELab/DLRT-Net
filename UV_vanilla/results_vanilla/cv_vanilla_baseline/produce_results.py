...
data_dlrt = pd.read_csv('_running_data_dlrt0dlrt_lenet_baseline.csv')[['epoch','test_accuracy(%)']]
data_vanilla = pd.read_csv('_running_data_vanilla0vanilla_lenet_baseline.csv')[['epoch','test_accuracy(%)']]
mean = data_dlrt.copy()
mean_vanilla = data_vanilla.copy()
for cv in range(1,10):
    data = pd.read_csv('_running_data_dlrt'+str(cv)+'dlrt_lenet_baseline.csv')[['epoch','test_accuracy(%)']]
    data_dlrt = pd.concat([data_dlrt,data])
    data_UV = pd.read_csv('_running_data_vanilla'+str(cv)+'vanilla_lenet_baseline.csv')[['epoch','test_accuracy(%)']]
    mean+=data
...
ratio = 0.5
plt.savefig("crossval_acc_dlrt_vs_UV_baseline.pdf", dpi=500,format = 'pdf')
plt.clf()
...
data_dlrt = pd.read_csv('_running_data_dlrt0dlrt_lenet_baseline.csv')[['epoch','train_loss']]
data_vanilla = pd.read_csv('_running_data_vanilla0vanilla_lenet_baseline.csv')[['epoch','train_loss']]
mean = data_dlrt.copy()
mean_vanilla = data_vanilla.copy()
for cv in range(1,10):
    data = pd.read_csv('_running_data_dlrt'+str(cv)+'dlrt_lenet_baseline.csv')[['epoch','train_loss']]
    data_dlrt = pd.concat([data_dlrt,data])
    data_UV = pd.read_csv('_running_data_vanilla'+str(cv)+'vanilla_lenet_baseline.csv')[['epoch','train_loss']]
    mean+=data
    mean_vanilla+=data_UV
...
y_low, y_high = ax.get_ylim()
ratio = 0.5
plt.savefig("crossval_loss_dlrt_vs_UV_baseline.pdf", dpi=500,format = 'pdf')
plt.clf()
