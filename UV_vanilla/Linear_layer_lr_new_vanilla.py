...
class Linear(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
                 device=None, dtype=None,rank = None,fixed = False,load_weights = None,step = 'S',full_rank_construct = False,decay=1) -> None:
        ...
        if self.lr and not full_rank_construct:
            ...
            if not self.fixed:   # initialization for dlrt adaptive
                ...
                U = U.to(device)
                V = V.to(device)             
                self.S_hat = torch.nn.Parameter(torch.diag(torch.sqrt(s_ordered)).to(device),requires_grad = False)      
                self.U = torch.nn.Parameter((U[:,:self.rank]@self.S_hat).to(device) ,requires_grad=True)                                    
                self.V = torch.nn.Parameter((V[:,:self.rank]@self.S_hat).to(device),requires_grad=True)
                self.K = torch.nn.Parameter(torch.randn(self.out_features,self.rank).to(device),requires_grad = False)
                self.L = torch.nn.Parameter(torch.randn(self.in_features,self.rank).to(device),requires_grad = False)
