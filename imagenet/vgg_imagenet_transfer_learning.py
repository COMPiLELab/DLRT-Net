...
from models_folder.vgg_transferlearning import VGG,VGG_types
import torch
...
import torchvision.models as tchm
...
parser.add_argument('--net_name', type=str, default='vgg_imagenet_transfer_learning_', metavar='NET_NAME',
...
NN = tchm.vgg16_bn(pretrained = True)
weights = NN.state_dict()
##################################################create new neural network
NN = VGG(num_classes = num_classes,
    architecture=VGG_types["VGG16"],device = device,full_rank_reconstruct = True
).to(device)

new_state_dict = dict()
for i,(k,w) in enumerate(weights.items()):
    new_key = list(NN.state_dict().keys())[i]
    new_state_dict[new_key] = w


NN.load_state_dict(new_state_dict)
NN.to_low_rank()
for l in NN.layer:
    if isinstance(l,Conv2d_lr) or isinstance(l,torch.nn.BatchNorm2d):
...
path = './results/vgg/'
# optimizer = dlr_opt(NN, tau=args.lr, theta=args.theta,name = 'sgd',KLS_optim = torch.optim.SGD,momentum = args.momentum)
# scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer.integrator)
############################################## 
